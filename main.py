...
def getMap():
    filesDir = os.getcwd() + "/new york data/"
...
# Day corresponds to the last of the df subtracted by day.
def getTemp(points, weatherData, day):
    temp = []
    
    for i in range(len(points)):
        df = weatherData[i - 1][0]
        avg = (df["tmax"].iloc[-day] + df["tmin"].iloc[-day]) / 2
...
for i in range(1, 8):
    ...
    values = getTemp(points, weatherData, i)
